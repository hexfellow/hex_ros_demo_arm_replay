...
arm_replay provides several launch files for one-command startup of different scenarios (the trajectory JSON path is configured in `config/<ros_version>/replay_param.yaml`):

**ROS 1:**

```shell
# Start only the replay node
roslaunch hex_ros_arm_replay arm_replay.launch

# Full real robot: keyboard teleop + arm driver + trajectory replay
roslaunch hex_ros_arm_replay real_replay.launch

# Full simulation: sim environment + keyboard teleop + trajectory replay
...
```
> Make sure your parameters are configured correctly.
